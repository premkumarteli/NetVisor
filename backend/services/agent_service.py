...
class AgentService:
    ...
    def get_agents_summary(
        ...
        for agent in agents:
            ...
            qd = int(
                agent.get("upload_queue_depth")
                or agent.get("inspection_queue_size")
                or 0
            )
            ...
            if is_online:
                has_failures = (
                    int(agent.get("upload_failures") or 0) > 0
                    or int(agent.get("upload_consecutive_failures") or 0) > 0
                    or int(agent.get("inspection_upload_failures") or 0) > 0
                )
                has_queue = qd > 0
                has_error = bool(
                    agent.get("last_upload_error")
                    or agent.get("capture_error_category")
                    or agent.get("inspection_last_error")
                )
                integrity_unverified = (
                    agent.get("integrity_status") != "verified"
                )
                if has_failures or has_queue or has_error or integrity_unverified:
                    degraded += 1
