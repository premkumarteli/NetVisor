...
def get_device_web_activity_groups(
    device_ip: str,
    limit: int = 50,
    current_user: dict = Depends(require_org_admin),
    conn = Depends(get_db),
...
def get_global_web_activity_groups(
    limit: int = 50,
    current_user: dict = Depends(require_org_admin),
    conn = Depends(get_db),
):
...
def get_device_inspection_status(
    device_ip: str,
    current_user: dict = Depends(require_org_admin),
    conn = Depends(get_db),
):
