...
def get_dashboard_overview(
    ...
    return cached_response(
        cache_key,
        3,
        lambda: live_telemetry_store.get_overview_stats(organization_id=org_id, db_conn=conn)
    )
...
def get_dashboard_activity(
    ...
    org_id = current_user.get("organization_id")
    cache_key = f"dashboard:activity:{org_id}:{limit}"
    return cached_response(
        cache_key,
        2,
        lambda: dashboard_service.get_recent_activity(conn, organization_id=org_id, limit=limit)
    )
...
def get_traffic_history(
    ...
    org_id = current_user.get("organization_id")
    cache_key = f"dashboard:traffic:{org_id}:{hours}:{resolution}:{window}"
    return cached_response(
        cache_key,
        5,
        lambda: dashboard_service.get_traffic_history(
            conn,
            hours=hours,
...
def get_device_stats(
    ...
    org_id = current_user.get("organization_id")
    cache_key = f"dashboard:devicestats:{org_id}:{limit}"
    return cached_response(
        cache_key,
        3,
        lambda: dashboard_service.get_device_activity_stats(conn, limit=limit, organization_id=org_id)
    )
...
def get_dashboard_bundle(
    ...
    org_id = current_user.get("organization_id")
    cache_key = f"dashboard:bundle:{org_id}:{limit}"
    ...
    return cached_response(cache_key, 3, _fetch_bundle)
