"""Gateway-side mTLS certificate management.
...
logger = logging.getLogger("netvisor.gateway.mtls")


class GatewayMTLS:
    """Manages the gateway's mTLS client certificate lifecycle."""
    ...
    def status_info(self) -> dict:
        ...
        return {
            "gateway_has_certificate": self.has_certificate(),
            "gateway_needs_renewal": self.needs_renewal(),
            "gateway_cert_serial": self.cert_serial(),
            "gateway_cert_fingerprint": self.cert_fingerprint(),
            "gateway_cert_expires_at": self.cert_expires_at(),
        }
    ...
    def generate_csr(self) -> bytes:
        ...
        subject = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, self.gateway_id),
        ...
        logger.info("Generated CSR for gateway_id=%s", self.gateway_id)
        return csr_pem
