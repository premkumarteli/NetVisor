...
- keep the gateway-specific live defaults (`NETVISOR_GATEWAY_FLOW_FLUSH_INTERVAL_SECONDS=1.5`, `NETVISOR_GATEWAY_UPLOAD_INTERVAL_SECONDS=1`, `NETVISOR_GATEWAY_DISCOVERY_SECONDS=15`) when you want BYOD device/app pages to update quickly
- keep `NETVISOR_GATEWAY_ACTIVE_TYPE_PROBE=false` unless you intentionally want slower active port probes for device type detection
