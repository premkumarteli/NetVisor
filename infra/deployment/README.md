...
- `infra/deployment/docker-compose.yml` now mounts only canonical runtime assets from the current repo layout
...
- from the repo root, start it with `docker compose --env-file .env -f infra/deployment/docker-compose.yml up -d`

Server production bundle:

- `infra/deployment/server/docker-compose.yml` is copied into the generated server bundle as `docker-compose.yml`
...
- `infra/deployment/server/Caddyfile` is copied into the bundle root as `Caddyfile`
