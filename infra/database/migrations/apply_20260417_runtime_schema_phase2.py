...
def main() -> None:
    ...
    try:
        ...
        if not column_exists(cursor, "devices", "agent_id"):
            _execute(
                "ALTER TABLE devices ADD COLUMN agent_id VARCHAR(100) NULL AFTER organization_id",
                "devices.agent_id",
                ignore_errnos=(DUPLICATE_COLUMN_ERROR,),
            )
        if not index_exists(cursor, "devices", "idx_devices_agent_org_last_seen"):
            _execute(
                "CREATE INDEX idx_devices_agent_org_last_seen ON devices (agent_id, organization_id, last_seen)",
                "devices.idx_devices_agent_org_last_seen",
                ignore_errnos=(DUPLICATE_KEY_ERROR,),
            )
