...
const DashboardPage = () => {
  ...
  const trafficChartData = useMemo(() => {
    const normalized = trafficHistory
      .map((entry) => ({
        label: (String(entry.hour || '').split(' ').pop() || '').slice(0, 5),
        value: parseByteValue(entry.byte_count || 0) / (1024 * 1024),
      }))
          ...
          <section className="cinematic-panel cinematic-panel--traffic">
            ...
            <div className="cinematic-panel__header">
              <div>
                ...
                <p>Network traffic over the last 24 hours.</p>
              </div>
              <StatusBadge tone="accent" icon="ri-line-chart-line">24 hour window</StatusBadge>
            </div>
            <TrafficChart data={trafficChartData} height={250} />
          </section>
